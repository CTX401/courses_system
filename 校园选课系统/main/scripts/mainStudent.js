document.addEventListener("DOMContentLoaded",()=>{
    ...
    async function loadCourses(searchText = "", option = ""){
        ...
        Object.keys(gc.data).forEach(code => {
            ...
            tr.innerHTML = `
                <td style="text-align:center">${courseIndex}</td>
                <td>${course}</td>
        ...
        tbody.addEventListener("click", startPop);        
    };

    function startPop(target){
        if(target.target.classList.contains("chooseClass")){
            const row = target.target.closest("tr");
            showPopUp(row.cells[1].innerText);
        }
    }
